...
const BOOT_LINES = [
  ...
  {
    text: "> ERR: HANDSHAKE TIMEOUT - RETRYING...",
    cls: "boot-line-err",
    pause: 400,
  },
  ...
  {
    text: "> LOADING SECTOR DATA............. 12 SECTORS",
    cls: "boot-line-ok",
  },
  ...
  {
    text: "> WARNING: THIS TERMINAL CONSUMES REAL RESOURCES",
    cls: "boot-line-warn",
  },
  { text: "> . . .", cls: "" },
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("uncomfortable-close")
...
function updateTicker() {
  ...
  const p = el("tick-power");
  if (p) p.textContent = s.power.toFixed(1);
  const w = el("tick-water");
  if (w) w.textContent = s.water.toFixed(0);
  const c = el("tick-compute");
  if (c) c.textContent = s.compute.toFixed(2);
  const y = el("tick-systems");
  if (y) y.textContent = s.systems.toLocaleString();
}
...
function updateFooter() {
  ...
  const pow = el("cost-power");
  if (pow) pow.textContent = s.power.toFixed(4);
  const wat = el("cost-water");
  if (wat) wat.textContent = s.water.toFixed(1);
  const co2 = el("cost-co2");
  if (co2) co2.textContent = s.co2.toFixed(4);
  const hci = el("humans-checkin");
  if (hci) hci.textContent = h.checkins.toLocaleString();
  const hto = el("humans-total");
  if (hto) hto.textContent = h.population.toLocaleString();
}
