...
function updateGauges() {
  ...
  // Slight drift
  OVERRIDE_STATE.watcher.power    = Math.max(5,  Math.min(95, s.power    + (Math.random() - 0.55) * 0.3));
  OVERRIDE_STATE.watcher.water    = Math.max(5,  Math.min(95, s.water    + (Math.random() - 0.6)  * 0.3));
  OVERRIDE_STATE.watcher.exposure = Math.max(50, Math.min(99, s.exposure + (Math.random() - 0.4)  * 0.2));
}
